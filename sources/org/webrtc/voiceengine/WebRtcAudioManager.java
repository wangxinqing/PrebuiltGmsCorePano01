...
public class WebRtcAudioManager {
    ...
    private void dispose() {
        String valueOf = String.valueOf(WebRtcAudioUtils.getThreadInfo());
        Logging.a(TAG, valueOf.length() == 0 ? new String("dispose") : "dispose".concat(valueOf));
        if (this.initialized) {
    ...
    private static int getMinInputFrameSize(int i, int i2) {
        return AudioRecord.getMinBufferSize(i, i2 == 1 ? BITS_PER_SAMPLE : 12, 2) / (i2 + i2);
    }
    ...
    private boolean init() {
        String valueOf = String.valueOf(WebRtcAudioUtils.getThreadInfo());
        Logging.a(TAG, valueOf.length() == 0 ? new String("init") : "init".concat(valueOf));
        if (this.initialized) {
            return blacklistDeviceForAAudioUsage;
        }
        String modeToString = WebRtcAudioUtils.modeToString(this.audioManager.getMode());
        Logging.a(TAG, modeToString.length() == 0 ? new String("audio mode is: ") : "audio mode is: ".concat(modeToString));
    ...
    private boolean isAAudioSupported() {
        Logging.c(TAG, "AAudio support is currently disabled on all devices!");
        return false;
    }
